...
正規表現ベースのマスキングでは、以下のような曖昧な表現に対応できません。しかし、LLMなら文脈を理解して「山本太郎さん」、「<taro.yamada@example.com>」が個人情報であることを認識し、適切にマスキングできます。

![ai_mask](/images/20251020_AI%20Orientedを目指したIVRyのDatabricksによるAI活用事例と、それを支える運用体制.png)

これは**プロダクトのデータパイプラインに組み込まれ、毎日自動で実行**されており、安心安全にデータ活用することができます。
...
Databricksを利用した際に自動で取得されるシステムテーブルがありま  す。
