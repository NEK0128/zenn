...
Keynoteを貫いていたのは「**AIに足りないのは知能ではなく、コンテキストだ**」というメッセージでした。CEOのAli Ghodsiは「十分に賢いAIはもう誰でも使える状態だ。それでも企業で活用が進まないのは、データにコンテキストが与えられていないからだ」と話していました。今年の軸は「AIの性能」から「**AIを信頼してビジネスで使えるか**」へ移ったと感じます。
...
ここからは自分が感じたことについてお話しします。
今回のSummitを通して、伝えたいことが一貫してはっきりしていました。

**AI時代の勝者は、派手なAIアプリを作る企業ではなく、AIが正しく働くためのデータと業務の土台を握る企業になり、基盤をどれくらい作り込めるかが肝になってきます。** モデルはもう十分賢く、差がつくのは、そのモデルに「自社のビジネスとは何か」を正しく渡せるかどうかです。

今は、テーブルを渡せばAIはSQLを書けます。でも「売り上げはどこまでを含むのか」「ID同士をどう紐づけるのか」といった業務文脈がないと、AIはもっともらしいけれどズレた答えを出してしまいます。こうしたコンテキストと業務理解を握ることが、より重要になってきます。
...
社内ベンチマークでは初回試行で84.5%に正答し、汎用コーディングエージェント（25〜52%）を大きく上回ったと紹介されていました。
