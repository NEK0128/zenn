...
社内向けデータアプリを作ると、「部署ごと・チームごとに見せるデータを分けたい」という要望はよく発生します。ところが、Databricks Apps をデフォルト設定のまま作ると、そのアプリにアクセスできる人は全員が同じカタログを閲覧できてしまいます。
...
1. ユーザーがアプリにアクセスすると、Databricks がログインユーザーのアクセストークンをリクエストヘッダー `x-forwarded-access-token` でアプリに渡す
...
- 仕様（ヘッダー名・スコープ・有効化手順）は変わる可能性があります。実装時は公式ドキュメントの最新版を確認してください。
